      ...
      <div className="flex flex-col gap-4 rounded-3xl border border-border bg-surface p-6 shadow-sm sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-text-secondary">
            {week.startDate} &middot; {Math.round(totalMinutes / 60)}h {totalMinutes % 60}m scheduled
          </p>
        </div>
      ...
      {Object.keys(subjectMinutes).length > 0 ? (
        <div className="flex flex-wrap gap-3">
          {Object.entries(subjectMinutes).map(([subject, mins]) => (
            ...
            >
              {subject}: {Math.round(mins / 60)}h {mins % 60}m
            </span>
          ))}
        </div>
      ) : null}
          ...
          if (!slots.length) {
            return (
              ...
              >
                ...
                <p className="mt-4 text-center text-xs italic text-text-muted">Free</p>
              </div>
            );
          }

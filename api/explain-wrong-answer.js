...
import { createLogger, withTimeout, retry, validateExplanation } from './_lib/forge-integrity.js';
...
async function callGeminiWithValidation(prompt) {
  ...
  const result = await withTimeout(
    retry(() => googleAI.models.generateContent({ model: geminiModel, contents: prompt }), { maxRetries: 3, baseDelay: 1000, logger: log }),
    15000
  );
